...
class ProductAPI {
  ...
  static async toggleProductStatus(id: string, isActive: boolean) {
    const response = await fetch(`${this.baseURL}/api/admin/products/${id}/toggle`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  static async duplicateProduct(id: string) {
    const response = await fetch(`${this.baseURL}/api/admin/products/${id}/duplicate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
    return response.json();
...
}> = ({ product, onEdit, onDelete, onCopy, onDuplicate, onToggleStatus }) => {
  const qualityColors = {
...
export default function TossProductManagement() {
  ...
  const handleDuplicate = async (id: string) => {
    try {
      ...
      if (response.success) {
        console.log('서비스가 복제되었습니다:', response.data.name);
        loadProducts(); // 목록 새로고침
      } else {
        console.error('서비스 복제 실패:', response.message);
      }
    } catch (error) {
      console.error('서비스 복제 실패:', error);
    }
  };
  ...
  const handleToggleStatus = async (id: string, isActive: boolean) => {
    try {
      ...
      if (response.success) {
        console.log(`상품 ${isActive ? '활성화' : '비활성화'} 완료`);
        loadProducts(); // 목록 새로고침
      } else {
        console.error("상품 상태 변경 실패:", response.message);
      }
    } catch (error) {
